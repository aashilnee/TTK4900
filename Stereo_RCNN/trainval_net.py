        ...
        if self.data_aug:
            for sample_train in train_dataset:
                ...
                augmentation = iaa.Sequential([
                    ...
                    # Perform methods below only sometimes (30% of images)
                    iaa.Sometimes(0.7,
                ...
                while t <= 10:  # 10 images created from 1 image
                    # Equal augmentation on left and right image
                    aug_det = augmentation.to_deterministic()
                    # Augmenting images and corresponding bounding boxes
        ...
        for step_, data_ in enumerate(validation_loader):
            ...
            log_string_val(
                '[epoch %2d][iter %4d/%4d] validation_loss: %.4f, lr: %.2e' % (
                    epoch, step_, iters_per_epoch, val_loss.item(), lr))
...
if __name__ == '__main__':
    ...
    log_info_val = open((output_dir + 'val_log.txt'), 'w')
    ...
    dataset = LarvaeDataset(file_path=train_dir, data_aug=True)
    ...
    training_loader = torch.utils.data.DataLoader(dataset_train, batch_size=1, shuffle=True)
    ...
    if args.resume:
        load_name = os.path.join(output_dir,
                                 'stereo_rcnn_epoch_1300_loss_-44.893638610839844.pth'.format(args.checkepoch,
                                                                                              args.checkpoint))
