...
if __name__ == '__main__':
    ...
    load_name = os.path.join(input_dir,
                             'stereo_rcnn_epoch_100_loss_-11.447897911071777.pth')
    ...
    with torch.no_grad():
        ...
        video_out = cv2.VideoWriter('/Users/aashi/Documents/videos/out/test_sort_new.mp4', fourcc, 20.0, (2560, 720))
        ...
        with open(path + '3D_pos.txt', 'w') as f:
            ...
            for frame_num in tqdm(range(1000, 2000)): # 1000-2000
                ...
                if frame_num_1 % 25 == 0:
                    for key in dictionary.keys():
                        ...
                        if len(dictionary[key]) == 25:
                            ...
                            if key in dictionary_velocity:
                                dictionary_velocity[key].append(velocity)

                            if not key in dictionary_velocity:
                                dictionary_velocity[key] = []
                                dictionary_velocity[key].append(velocity)


                            print("DICTIONARY VELOCITY: ", dictionary_velocity)

                            acceleration = 0

                            if len(dictionary_velocity[key]) == 2:
                                print("two values saved for velocity")

                                velocity_0 = dictionary_velocity[key][0]
                                velocity_1 = dictionary_velocity[key][1]
                                ...
                                dictionary_velocity[key] = [velocity_1]

                            if key == 1:
                                x_list.append(x_1)
                                y_list.append(y_1)
                                z_list.append(z_1)
                                velocity_item.append(velocity)
                                acceleration_item.append(acceleration)
                ...
                # Stored values for one fish
                print("X-position for fish1: ", x_list)
                print("Y_position for fish1: ", y_list)
                print("Z-position for fish1: ", z_list)
                print("Velocity for fish1: ", velocity_item)
                print("Acceleration for fish1: ", acceleration_item)
                ########################

                # Stored velocity and acceleration for all fish
                print("velocity for all fish: ", velocity_list)
                print("acceleration for all fish: ", acceleration_list)
